...
def prop_func_angular_spectrum_pyffs(out_shift):
    return angular_spectrum(
        u_in=u_in, wv=wv, d1=d1, dz=dz, d2=d2, bandlimit=True, out_shift=out_shift, pyffs=True
    )[0]


start_time = time.time()
u_out, x2, y2 = rect_tiling(
...
start_time = time.time()
u_out, x2, y2 = rect_tiling(
    N_in=N_in, N_out=N_out, L=L_in, n_tiles=n_tiles, prop_func=prop_func_angular_spectrum_pyffs
)
